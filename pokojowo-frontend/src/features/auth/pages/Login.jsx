...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) {
      ...
      if (!user?.role || user.role.length === 0 || (user.role.length === 1 && user.role[0] === 'User')) {
      ...
      if (!user?.isProfileComplete) {
        ...
        if (isTenant) {
          navigate('/profile-completion/tenant', { replace: true });
        } else if (isLandlord) {
          navigate('/profile-completion/landlord', { replace: true });
        } else {
